...
def create_case(cutoff: date = CUTOFF) -> CreateCase:
    return CreateCase(question=QUESTION, cutoff=cutoff, document=aurora_document())


def scripted_application(
    root: Path,
    *,
    ...
    if candidates is None:
        candidates = {SOURCE_ID: aurora_candidate()}
    return FinResearchOps(
        artifact_root=root,
        model=ScriptedModelAdapter(candidates),
        clock=clock,
    )
...
class ApplicationTest(unittest.TestCase):
    def test_application_return_dataclasses_keep_exact_fields(self) -> None:
        self.assertEqual(
            (
                "workpaper_ref",
                "run_ref",
            ...
            tuple(field.name for field in fields(WorkpaperView)),
        )
        self.assertEqual(
            (
                "operation",
            ...
            tuple(field.name for field in fields(ApplicationOutcome)),
        )

    def test_application_schemas_are_closed_and_versioned(self) -> None:
        expected = {
            ...
            "workpaper.v3.schema.json": (
                "urn:finresearchops:schema:workpaper:v3",
                "finresearchops.workpaper/v3",
            ),
            ...
            "replay-record.v3.schema.json": (
                "urn:finresearchops:schema:replay-record:v3",
                "finresearchops.replay-record/v3",
            ),
        }
        ...
        replay_record = json.loads((SCHEMA_DIR / "replay-record.v3.schema.json").read_bytes())
        self.assertEqual(
            "urn:finauditgate:schema:replay-report:v5",
            replay_record["properties"]["report"]["$ref"],
        )
        workpaper = json.loads((SCHEMA_DIR / "workpaper.v3.schema.json").read_bytes())
        self.assertEqual(
            "urn:finauditgate:schema:run-ref:v1",
            workpaper["properties"]["run_ref"]["$ref"],
        )
        self.assertEqual(
            "^runs/[0-9a-f]{64}/(?:attempts|model-trace)\\.json$",
            workpaper["properties"]["trace_summary_ref"]["pattern"],
        )
        self.assertEqual(
            ["ATTEMPTS_RECORDED", "MODEL_TRACE_BOUND"],
            workpaper["properties"]["trace_summary_status"]["enum"],
        )
    ...
    def test_application_interface_and_commands_are_closed(self) -> None:
        ...
        self.assertEqual(("self", "command"), tuple(signature(FinResearchOps.handle).parameters))
        self.assertEqual(
            ("self", "case_ref"),
            tuple(signature(FinResearchOps.read_case).parameters),
        )
        self.assertEqual(
            ("question", "cutoff", "document", "answer_contract", "risk_class", "mode"),
    ...
    def test_create_run_and_read_never_turn_machine_accept_into_approval(
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            application = scripted_application(
                Path(temporary_directory),
                clock=lambda: utc(12, 0),
            )
            command = create_case()
            ...
            self.assertEqual(
                f"runs/{analysis.run_ref.run_id}/attempts.json",
            ...
            self.assertEqual("ATTEMPTS_RECORDED", view.workpapers[0].trace_summary_status)
    ...
    def test_approved_case_exports_one_proposal_and_replays_after_restart(
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            application = scripted_application(root, clock=lambda: utc(12, 1))
            created = application.handle(create_case())
            analysis = application.handle(RunAnalysis(created.case_ref))
            ...
            packet = application.handle(ExportChangePacket(created.case_ref, analysis.run_ref))
    ...
    def test_non_accept_cannot_be_approved_or_exported_and_return_can_rerun(
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            application = scripted_application(
                Path(temporary_directory),
                candidates={
                    SOURCE_ID: invalid_value_candidate(),
                    f"{SOURCE_ID}:analysis-2": aurora_candidate(),
                },
                clock=lambda: utc(12, 2),
            )
            created = application.handle(create_case())
            first = application.handle(RunAnalysis(created.case_ref))

            self.assertIsNot(Decision.ACCEPT, first.machine_decision)
            ...
            self.assertEqual("MACHINE_DECISION_NOT_APPROVABLE", approval_error.exception.code)
            with self.assertRaises(ApplicationError) as export_error:
                application.handle(ExportChangePacket(created.case_ref, first.run_ref))
            self.assertEqual("CASE_NOT_ELIGIBLE_FOR_EXPORT", export_error.exception.code)
            pending = application.read_case(created.case_ref)
            self.assertEqual(CaseStatus.AWAITING_REVIEW, pending.status)
            self.assertIsNot(Decision.ACCEPT, pending.latest_machine_decision)
            ...
            with self.assertRaises(ApplicationError):
                application.handle(ExportChangePacket(created.case_ref, second.run_ref))
    ...
    def test_closed_commands_and_case_run_ownership_fail_without_state_change(
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            application = scripted_application(
                Path(temporary_directory),
                clock=lambda: utc(12, 3),
            )
            first_case = application.handle(create_case())
            second_case = application.handle(create_case(LATER_CUTOFF))
            ...
            self.assertEqual("REVIEW_RUN_IS_NOT_CURRENT", cross_case_error.exception.code)
            ...
            with self.assertRaises(ApplicationError) as stale_export_error:
                application.handle(
                    ExportChangePacket(first_case.case_ref, second_run.run_ref)
                )
            self.assertEqual("EXPORT_RUN_IS_NOT_CURRENT", stale_export_error.exception.code)
    ...
    def test_tampered_application_or_core_artifact_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory)
            application = scripted_application(root)
            created = application.handle(create_case())
            analysis = application.handle(RunAnalysis(created.case_ref))
            workpaper_path = (
                root
            ...
            outcome_path = root / "core" / "runs" / analysis.run_ref.run_id / "outcome.json"
            ...
            self.assertIn(
                ...
                {"ARTIFACT_READ_FAILED", "CASE_TRANSACTION_COMMIT_INVALID"},
            )
    ...
    def test_content_addressed_workpaper_cannot_replace_core_reason_codes(
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory)
            application = scripted_application(root)
            created = application.handle(create_case(date(2026, 2, 14)))
            analysis = application.handle(RunAnalysis(created.case_ref))
            case_directory = root / "application" / "cases" / created.case_ref
            workpaper_path = case_directory / "workpapers" / f"{analysis.workpaper_ref}.json"
            ...
            forged_ref = "workpaper-" + sha256_hex(canonical_json_bytes(body))
            payload["workpaper_ref"] = forged_ref
            (case_directory / "workpapers" / f"{forged_ref}.json").write_bytes(
                canonical_json_bytes(payload)
            )
            event_path = case_directory / "events" / "00000001.json"
            ...
            self.assertIn(
                ...
                {"WORKPAPER_CORE_MISMATCH", "CASE_TRANSACTION_PUBLISHED_CONTENT_INVALID"},
            )
    ...
    def test_analysis_adapter_failure_stays_inside_application_error_family(
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            application = FinResearchOps(artifact_root=Path(temporary_directory))
            created = application.handle(create_case())

            with self.assertRaises(ApplicationError) as error:
                application.handle(RunAnalysis(created.case_ref))
            self.assertEqual("ANALYSIS_EXECUTION_FAILED", error.exception.code)
            self.assertEqual(
                CaseStatus.CREATED,
    ...
    def test_deleted_tail_review_or_analysis_event_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory)
            application = scripted_application(root, clock=lambda: utc(12, 5))
            created = application.handle(create_case())
            analysis = application.handle(RunAnalysis(created.case_ref))
            reviewed = application.handle(
                SubmitReview(
            ...
            case_directory = root / "application" / "cases" / created.case_ref
            event_directory = case_directory / "events"
            (event_directory / "00000002.json").unlink()
            (case_directory / "reviews" / f"{reviewed.review_ref}.json").unlink()

            with self.assertRaises(ApplicationError) as review_tail_error:
                application.read_case(created.case_ref)
            self.assertIn(
                review_tail_error.exception.code,
                {"CASE_JOURNAL_HEAD_MISMATCH", "CASE_TRANSACTION_COMMIT_INVALID"},
            )
            with self.assertRaises(ApplicationError):
                application.handle(
                    SubmitReview(
            ...
            (case_directory / "workpapers" / f"{analysis.workpaper_ref}.json").unlink()
    ...
    def test_foreign_case_artifacts_in_case_directory_fail_closed(self) -> None:
        for kind in ("workpapers", "reviews"):
            with self.subTest(kind=kind), tempfile.TemporaryDirectory() as temporary_directory:
                root = Path(temporary_directory)
                application = scripted_application(root, clock=lambda: utc(13, 5))
                case_a = application.handle(create_case())
                case_b = application.handle(create_case(LATER_CUTOFF))
                analysis_b = application.handle(RunAnalysis(case_b.case_ref))
                if kind == "workpapers":
                    artifact_ref = analysis_b.workpaper_ref
                else:
                    artifact_ref = application.handle(
                        SubmitReview(
                            case_b.case_ref,
                            analysis_b.run_ref,
                            ReviewAction.APPROVE,
                ...
                source = (
                    root / "application" / "cases" / case_b.case_ref / kind
                    / f"{artifact_ref}.json"
                )
                destination = (
                    root / "application" / "cases" / case_a.case_ref / kind / source.name
                )
            ...
            self.assertEqual("CASE_ARTIFACT_MEMBERSHIP_INVALID", error.exception.code)

    def test_analysis_recovers_after_journal_head_publish_failure(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory)
            application = scripted_application(root)
            created = application.handle(create_case())
            ...
            reopened = FinResearchOps(artifact_root=root).read_case(created.case_ref)
            self.assertEqual(CaseStatus.CREATED, reopened.status)
            reopened_application = scripted_application(root)
    ...
    def test_analysis_transition_recovers_at_every_publication_boundary(self) -> None:
        for boundary in ("intent", "workpaper", "event", "head", "commit"):
    ...
    def test_review_transition_recovers_at_every_publication_boundary(self) -> None:
        for boundary in ("intent", "review", "event", "head", "commit"):
            with self.subTest(boundary=boundary), tempfile.TemporaryDirectory() as temporary_directory:
                root = Path(temporary_directory)
                clock = lambda: utc(13, 0)
                application = scripted_application(root, clock=clock)
                created = application.handle(create_case())
                analysis = application.handle(RunAnalysis(created.case_ref))
                ...
                reopened_application = FinResearchOps(artifact_root=root, clock=clock)
                reopened = reopened_application.read_case(created.case_ref)
                if boundary != "commit":
                    self.assertEqual(CaseStatus.AWAITING_REVIEW, reopened.status)
    ...
    def test_tampered_case_transaction_intent_fails_closed(self) -> None:
        ...
        self.assertIn(
            error.exception.code,
            {"CASE_TRANSACTION_IDENTITY_INVALID", "CASE_ARTIFACT_MEMBERSHIP_INVALID"},
        )

    def test_committed_transaction_cannot_recreate_deleted_material(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory)
            application = scripted_application(root)
            created = application.handle(create_case())
            analysis = application.handle(RunAnalysis(created.case_ref))
            workpaper_path = (
                root / "application" / "cases" / created.case_ref / "workpapers"
                / f"{analysis.workpaper_ref}.json"
            )
            workpaper_path.unlink()
    ...
    def test_read_does_not_authorize_an_injected_pending_review_intent(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory) / "original"
            clone = Path(temporary_directory) / "clone"
            clock = lambda: utc(13, 15)
            application = scripted_application(root, clock=clock)
            created = application.handle(create_case())
            analysis = application.handle(RunAnalysis(created.case_ref))
            ...
            clone_transactions = clone / "application" / "cases" / created.case_ref / "transactions"
            injected_intent = next(
                path
                for path in clone_transactions.glob("*.intent.json")
                if json.loads(path.read_bytes())["sequence"] == 2
            )
            original_transactions = root / "application" / "cases" / created.case_ref / "transactions"
            (original_transactions / injected_intent.name).write_bytes(injected_intent.read_bytes())
            ...
            self.assertFalse(
                any(
                    ...
                    for path in original_transactions.glob("*.commit.json")
                )
            )
            self.assertEqual(
                ["00000001.json"],
                [
                    path.name
                    for path in (
                        root / "application" / "cases" / created.case_ref / "events"
                    ).glob("*.json")
                ],
            )

    def test_commit_marker_without_published_transition_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory) / "original"
            clone = Path(temporary_directory) / "clone"
            clock = lambda: utc(13, 20)
            application = scripted_application(root, clock=clock)
            created = application.handle(create_case())
            analysis = application.handle(RunAnalysis(created.case_ref))
            shutil.copytree(root, clone)
            ...
            transaction_ref = json.loads(sequence_two_intent.read_bytes())["transaction_ref"]
            sequence_two_commit = clone_transactions / f"{transaction_ref}.commit.json"
            original_transactions = root / "application" / "cases" / created.case_ref / "transactions"
            for source in (sequence_two_intent, sequence_two_commit):
                (original_transactions / source.name).write_bytes(source.read_bytes())

            with self.assertRaises(ApplicationError) as error:
                FinResearchOps(artifact_root=root).read_case(created.case_ref)

        self.assertEqual("CASE_TRANSACTION_COMMIT_INVALID", error.exception.code)

    def test_pending_review_requires_the_same_public_command_to_resume(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory)
            first_clock = lambda: utc(13, 30)
            application = scripted_application(root, clock=first_clock)
            created = application.handle(create_case())
            analysis = application.handle(RunAnalysis(created.case_ref))
            ...
            reopened_application = FinResearchOps(artifact_root=root, clock=lambda: utc(15, 45))
        ...
        self.assertEqual("CASE_TRANSACTION_PENDING_CONFLICT", conflict.exception.code)
    ...
    def test_domain_invalid_pending_review_never_advances_head(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            review_ref = "review-" + sha256_hex(canonical_json_bytes(review_body))
            ...
            intent_body = {key: value for key, value in intent.items() if key != "transaction_ref"}
            transaction_ref = "transaction-" + sha256_hex(canonical_json_bytes(intent_body))
            intent["transaction_ref"] = transaction_ref
            case_directory = root / "application" / "cases" / created.case_ref
            (case_directory / "transactions" / f"{transaction_ref}.intent.json").write_bytes(
                canonical_json_bytes(intent)
            )
            head_path = case_directory / "journal-head.json"
            ...
            event_two_exists = (case_directory / "events" / "00000002.json").exists()

        self.assertEqual("REVIEW_RECORD_VALUE_INVALID", error.exception.code)
        self.assertEqual(head_before, head_after)
        self.assertFalse(event_two_exists)

    def test_pending_analysis_cannot_rehome_a_run_from_another_case(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory)
            application = scripted_application(root)
            early = application.handle(create_case())
            late = application.handle(create_case(LATER_CUTOFF))
            late_analysis = application.handle(RunAnalysis(late.case_ref))
            late_case_directory = root / "application" / "cases" / late.case_ref
            late_workpaper = json.loads(
                (late_case_directory / "workpapers" / f"{late_analysis.workpaper_ref}.json").read_bytes()
            )
            late_workpaper["case_ref"] = early.case_ref
            workpaper_body = {
                key: value for key, value in late_workpaper.items() if key != "workpaper_ref"
            }
            foreign_ref = "workpaper-" + sha256_hex(canonical_json_bytes(workpaper_body))
            late_workpaper["workpaper_ref"] = foreign_ref
            early_case_directory = root / "application" / "cases" / early.case_ref
            previous_head = json.loads((early_case_directory / "journal-head.json").read_bytes())
            event = {
            ...
            next_head = {
                ...
                "case_ref": early.case_ref,
                "event_count": 1,
                "tail_event_sha256": sha256_hex(canonical_json_bytes(event)),
            }
            intent_body = {
        ...
        self.assertEqual(canonical_json_bytes(previous_head), head_after)
        self.assertFalse(event_exists)

    def test_committed_transaction_rejects_forged_command_identity(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory)
            application = scripted_application(root)
            created = application.handle(create_case())
            ...
            original_intent_path = next(transaction_directory.glob("*.intent.json"))
            intent = json.loads(original_intent_path.read_bytes())
            original_ref = intent["transaction_ref"]
            original_commit_path = transaction_directory / f"{original_ref}.commit.json"
            intent["command_identity"] = {
                "schema_version": "forged.command/v999",
                "case_ref": created.case_ref,
            }
            intent_body = {key: value for key, value in intent.items() if key != "transaction_ref"}
            forged_ref = "transaction-" + sha256_hex(canonical_json_bytes(intent_body))
            ...
            (transaction_directory / f"{forged_ref}.intent.json").write_bytes(
                canonical_json_bytes(intent)
            )
            (transaction_directory / f"{forged_ref}.commit.json").write_bytes(
        ...
        self.assertEqual("CASE_TRANSACTION_COMMAND_INVALID", error.exception.code)

    def test_missing_non_tail_commit_receipt_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory)
            application = scripted_application(root, clock=lambda: utc(13, 55))
            created = application.handle(create_case())
            analysis = application.handle(RunAnalysis(created.case_ref))
            application.handle(
                SubmitReview(
    ...
    def test_unsealed_tail_cannot_coexist_with_a_later_pending_intent(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory)
            application = scripted_application(root, clock=lambda: utc(14, 0))
            created = application.handle(create_case())
            analysis = application.handle(RunAnalysis(created.case_ref))
            original_write_once = application_module.write_once
    ...
    def test_resumed_non_accept_review_preserves_gate_reason_codes(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory)
            application = scripted_application(root, clock=lambda: utc(14, 5))
            created = application.handle(create_case(date(2026, 2, 14)))
            analysis = application.handle(RunAnalysis(created.case_ref))
            command = SubmitReview(
                created.case_ref,
        ...
        self.assertEqual(("POST_CUTOFF_DOCUMENT",), resumed.reason_codes)
    ...
    def test_persistence_conflict_is_a_stable_application_error(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            with self.assertRaises(ApplicationError) as error:
                application.handle(command)
            self.assertEqual("APPLICATION_ARTIFACT_CONFLICT", error.exception.code)

    def test_concurrent_analysis_rechecks_state_before_any_second_commit(self) -> None:
        class BlockingAdapter:
            def __init__(self) -> None:
                self.entered = Event()
                self.release = Event()

            def propose(self, task: object, attempt_index: int = 0) -> ScriptedCandidate:
                self.entered.set()
                if not self.release.wait(timeout=5):
                    raise TimeoutError("test did not release blocking Adapter")
                return aurora_candidate()

        with tempfile.TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory)
            blocker = BlockingAdapter()
            first_application = FinResearchOps(artifact_root=root, model=blocker)
            second_application = scripted_application(root)
            created = first_application.handle(create_case())
            second_finished = Event()

            def second_run() -> object:
                try:
                    return second_application.handle(RunAnalysis(created.case_ref))
                finally:
                    second_finished.set()
            ...
            self.assertEqual("ANALYSIS_NOT_ALLOWED_IN_CURRENT_STATE", second_result.code)
    ...
    def test_concurrent_identical_replays_are_content_idempotent(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            with ThreadPoolExecutor(max_workers=worker_count) as executor:
                outcomes = tuple(executor.map(lambda _: replay_once(), range(worker_count)))

            self.assertEqual(1, len({outcome.replay_ref for outcome in outcomes}))
            self.assertEqual(1, len(application.read_case(created.case_ref).replays))
    ...
    def test_concurrent_identical_create_case_is_content_idempotent(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            with ThreadPoolExecutor(max_workers=worker_count) as executor:
                outcomes = tuple(executor.map(lambda _: create_once(), range(worker_count)))

            self.assertEqual(1, len({outcome.case_ref for outcome in outcomes}))
            view = FinResearchOps(artifact_root=root).read_case(outcomes[0].case_ref)
            self.assertEqual(CaseStatus.CREATED, view.status)

    def test_different_cases_can_atomically_share_one_document_blob(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory)
            commands = (create_case(), create_case(LATER_CUTOFF))
    ...
    def test_unlisted_model_trace_cannot_bind_a_workpaper(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory)
            document = aurora_document()
            task = AuditTask(
                task_id=document.source_id,
                question=QUESTION,
                cutoff=CUTOFF,
                document=document,
            )
            scripted = {SOURCE_ID: aurora_candidate()}
            existing_run = FinAuditGate(
                artifact_root=root / "core",
                model=ScriptedModelAdapter(scripted),
            ).run(task)
            run_directory = root / "core" / "runs" / existing_run.run_ref.run_id
            (run_directory / "model-trace.json").write_bytes(b"{}")
            core_replay = FinAuditGate(artifact_root=root / "core").replay(existing_run.run_ref)

            application = FinResearchOps(artifact_root=root, model=ScriptedModelAdapter(scripted))
            created = application.handle(
                CreateCase(question=task.question, cutoff=task.cutoff, document=document)
            )

            with self.assertRaisesRegex(ApplicationError, "CORE_ARTIFACT_SET_MISMATCH"):
                application.handle(RunAnalysis(created.case_ref))

            with self.assertRaises(ApplicationError):
                application.handle(ExportChangePacket(created.case_ref, existing_run.run_ref))

            view = application.read_case(created.case_ref)

        self.assertTrue(core_replay.consistent)
        self.assertEqual(CaseStatus.CREATED, view.status)
        self.assertEqual((), view.run_refs)
        self.assertEqual((), view.workpapers)

    def test_human_review_reason_reaches_pending_case(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            application = scripted_application(Path(temporary_directory))
            created = application.handle(create_case(date(2026, 2, 14)))
            analysis = application.handle(RunAnalysis(created.case_ref))
            view = application.read_case(created.case_ref)
